...
class Handler:
    ...
    def get_list_from_to(self, start_time, end_time):
        ...
        print(self.not_every_day_acts)
        self.every_day_acts = self.storage.get_every_day_acts()
        self.single_acts = self.storage.get_single_acts()
        res = []
        ...
        for obj in self.not_every_day_acts:
            if obj.start_time < start_time:
                while k < len(self.not_every_day_acts):
                    i = start_time
                    while i < end_time:
                        diff = i - datetime.timedelta(self.not_every_day_acts[k].start_time.day)
                        if diff.day % self.not_every_day_acts[k].days_counter == 0:
                            a = copy.copy(self.not_every_day_acts[k])  # true way
                            a.start_time = i
                            res.append(a)
                            i = i + datetime.timedelta(1)
                        k += 1
